...
/// Execute the id-run subcommand.
pub fn run_id_cmd(cmd: IdRunCmd) -> HxResult<i32> {
    let id = if cmd.pin {
        #[cfg(not(feature = "deterministic"))]
        {
            return Err(crate::error::HxError::Other(
                "id-run --pin requires the 'deterministic' feature".into(),
            ));
        }
        #[cfg(feature = "deterministic")]
        {
            ...
            let bs: serde_json::Value = serde_json::from_str(&raw).map_err(|e| {
                crate::error::HxError::Other(format!(
                    "id-run --pin: malformed bootstrap.json: {}",
                    e
                ))
            })?;
            let ts = bs
...
/// Execute the id-agent subcommand.
pub fn run_id_agent_cmd(cmd: IdAgentCmd) -> HxResult<i32> {
    ...
    let id = if cmd.pin {
        ...
        {
            return Err(crate::error::HxError::Other(
                "id-agent --pin requires the 'deterministic' feature".into(),
            ));
        }
        #[cfg(feature = "deterministic")]
        {
            ...
            let bs: serde_json::Value = serde_json::from_str(&raw).map_err(|e| {
                crate::error::HxError::Other(format!(
                    "id-agent --pin: malformed bootstrap.json: {}",
                    e
                ))
            })?;
            let ts = bs
